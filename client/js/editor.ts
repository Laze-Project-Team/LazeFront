...
$(() => {
  ...
  $('#btn-language').on('click', languageChange);

  // newProject
  $('#project-name').on('keyup', () =>
    $('#project-name').val()
      ? $('#setname-submit').prop('disabled', false)
  ...
  const adjustCanvasSize = (direction: 'x' | 'y') => {
    if (direction === 'x') {
      (document.querySelector('.editor-console') as HTMLElement)!.style.height =
        (canvas.clientWidth * 9) / 16 +
        document.querySelector('.editor-output-label')!.clientHeight +
        'px';
    } else {
      (document.querySelector('.editor-output') as HTMLElement)!.style.width =
        document.querySelector('.editor-console')!.clientWidth -
        (canvas.clientHeight * 16) / 9 +
        'px';
    }
    return true;
  };
  ...
  socket.on(
    'loadedFile',
    (result: { fileContent: string; logValue: string; style: string }) => {
...
socket.on(
  'output',
  (result: { value: string | string[]; style: 'log' | 'err' | 'info' }) =>
...
function save(content: string) {
  currentContents.filter((content) => content.path === currentFile)[0].content =
    content;
...
function contextmenu(contexts: contextObject[], e: Event) {
  ...
  for (const context of contexts) {
    const item = $.parseHTML(
      `<a class="contextmenu-item">${context.name}</a>`
    )[0];
    ...
    dir.value.forEach((subdir) => {
      ...
      if (subdir.type === 'folder') {
        ...
        file.addEventListener('contextmenu', (e) =>
          contextmenu(contextFolder(file), e)
        );
      ...
      if (subdir.type === 'file') {
        ...
        file.addEventListener('contextmenu', (e) =>
          contextmenu(contextFile(file), e)
        );
...
const fileSort = (
  a: { type: 'file' | 'folder'; name: string },
  b: { type: 'file' | 'folder'; name: string }
) => {
  if (a.type === b.type) {
...
function renameDir(oldPath: string, newFilename: string, newPath: string) {
  ...
  if (dirObj) {
    ...
    currentContents.filter((content) => content.path === oldPath)[0].path =
      newPath;
    $(`.exp-view li[data-path="${oldPath}"]`).text(newFilename);
    (
      document.querySelector(
        `.exp-view li[data-path="${oldPath}"]`
      )! as HTMLElement
      ...
      [
        {
          ...
          name: 'キャンセル',
          role: 'cancel',
          icon: '/assets/icons/icons.svg#cross',
        },
        {
          id: 'save',
          name: '保存しないで閉じる',
        ...
        },
        {
          ...
          name: '保存して閉じる',
          role: 'submit',
          icon: '/assets/icons/icons.svg#save',
        },
      ],
      () => {
...
const initialButtons = [
  ...
  },
  {
    ...
    name: '決定',
    role: 'submit',
    icon: '/assets/icons/icons.svg#check',
  },
];
...
$(() => {
  ...
  $(document).on('keydown', (e) =>
    e.key === 'Escape' ? $('#modal').removeClass('show') : null
  );
