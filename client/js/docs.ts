...
// ミラーリング
document.querySelector('#index-list .index-list')!.innerHTML =
  document.querySelector('.sidebar .index-list')!.innerHTML;
document.querySelector('.responsive-header .version-select')!.innerHTML =
  document.querySelector('.versions .version-select')!.innerHTML;

// version-select 同期
document
  .querySelectorAll('.version-select .input-field select')
  .forEach((selectElement) => {
    selectElement.addEventListener('change', (event) => {
      ...
      document
        .querySelectorAll('.version-select .input-field input')
        .forEach((element) => {
          // @ts-ignore
          element.value = event.target?.querySelector(
...
};
{
  ...
  renderer.code = (code, language) =>
    `<pre><div class="line-number">${getLineNumber(
      code
    )}</div><code class="codeblock-laze">${highlight(code)}</code></pre>`;
  renderer.link = (href, text, string) =>
    `<a href="${href}" class="mdlink">${string}</a>`;
  renderer.heading = (text, level) => {
    const div = document.createElement('div');
    div.innerHTML = text;
    let innerText = text;
    for (
      let element: HTMLElement | null = div;
    ...
    return `<h${level} id="${innerText}">${text}${
      level >= 1 && level <= 2
        ? '<a class="link-copy tooltipped" data-position="bottom" data-tooltip="' +
          'ここへのリンクをコピー' +
          '" data-href="' +
          currentHref +
          '#' +
...
function setMarkdownLink() {
  (
    document.querySelectorAll('a.mdlink') as NodeListOf<HTMLLinkElement>
  ).forEach((link) => {
    ...
    .then((res) => {
      ...
      if (!pop) {
        const newurl =
          ...
          `?version=${getVersion()}&href=${currentHref}`;
        history.pushState({ path: newurl }, '', newurl);
      }

      // スクロール初期化
      doc.scrollTo({ top: 0, left: 0 });

      // active変更
      document
        .querySelectorAll('.actived')
        .forEach((actived) => actived.classList.remove('actived'));
      document.querySelectorAll(`a[href="${href}"]`).forEach((actived) => {
        ...
        if (actived.classList.contains('collection-item')) {
          for (
            let element: HTMLElement | null = actived as HTMLElement;
      ...
      document
        ...
        .forEach((element) => {
          ...
          element.value = document.querySelector(
            `.version-select .input-field select option[value=${ver}]`
          ).innerHTML;
        });
    }
...
function parseMarkdown(content: string) {
  ...
  // 見出し一覧を表示
  document.querySelector('.document-outline > .content')!.innerHTML = headline(
    content
  )
  ...
  // 見出し一覧をスムーススクロールに
  document
    .querySelectorAll('.document-outline > .content > a')!
    .forEach((link) => {
      link.addEventListener('click', smoothScroll);
    });
  ...
  document.querySelectorAll('.link-copy').forEach((element) => {
    ...
    element.addEventListener('click', (e) => {
      ...
      // @ts-ignore
      dummyElement.innerText = `${location.origin}${
        location.pathname
      }?version=${getVersion()}&href=${element.dataset.href}`;
      dummyElement.select();
  ...
  setTimeout(() => {
    if (innerWidth < 993)
      M.TapTarget.getInstance(
        document.querySelector('.tap-target[data-target="index-list-trigger"]')!
      ).open();
  }, 1000);
}
...
document
  .getElementById('close-discovery')!
  .addEventListener('click', closeDiscovery);
...
main.addEventListener('scroll', () => {
  ...
  if (active)
    document
      .querySelector(`.document-outline > .content > a[href="#${active}"]`)
